...
3. tag 命名规则
  ...
  2. 临时紧急修复tag，各业务线和组件加`前缀fix加上对应的问题版本号，例如fix10.0.2，壳工程比较特殊
  3. 例如根据各个业务线对应fix10.0.2打出来的包，壳工程应该是AppStore10.0.3。 
  4. 壳工程所在仓库每次发AppStore，master分支或者fix分支必须有一个对应的tag号，例如AppStore10.0.2与苹果商店版本号对应，切记一定要是全量代码，包括pods部分和target工程配置，因为有部分代码可能会临时修改，没有包括在业务线的tag号中。
