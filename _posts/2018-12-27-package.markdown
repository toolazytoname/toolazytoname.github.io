...
~~~
  > Copying xxxxxxxMessageCenterLib from `/Users/xxxxxxx/Library/Caches/CocoaPods/Pods/External/xxxxxxxMessageCenterLib/ac0999518905dfaeb4412e75cb70102e` to `Pods/xxxxxxxMessageCenterLib`
-> Pre-downloading: `xxxxxxxLib` from `http://gitlab.xxxxxxx.com/WP/Mobile/IOS/xxxxxxxLib.git`, tag `0.4.5`
 > Git download
 > Git download
     $ /usr/bin/git clone http://gitlab.xxxxxxx.com/WP/Mobile/IOS/xxxxxxxLib.git /var/folders/7w/q65n0jy56nq42f3k0nlfnbm00000gn/T/d20181228-23832-zemqsr --template= --single-branch --depth 1 --branch 0.4.5
...
[!] Error installing xxxxxxxLib
[!] Failed to download 'xxxxxxxLib'.
~~~

虽然用到的文件是7M，但是pod update 会把整个目录都给拉下来，一整个目录一个多G，当然会失败了。我把 pods 目录给放到ignore 里面了，体量骤减。
